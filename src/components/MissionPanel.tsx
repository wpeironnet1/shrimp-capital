...
export function MissionPanel({state,onClaim}:{state:GameState;onClaim:(id:string)=>void}){
  ...
  return <View style={styles.frame} accessibilityLabel={`${mission.detail}. ${progress} of ${mission.target} complete.`}>
    <View style={[styles.headerBand,mobile&&styles.headerBandMobile]}>
      <PixelText style={styles.kicker}>MANDATE {missionIndex+1} / {missions.length}</PixelText>
    ...
    <View style={styles.card}>
      ...
      <View style={styles.progressRow}><PixelText style={styles.progressText}>{progress} / {mission.target}</PixelText></View>
      <View style={styles.track}><View style={[styles.fill,{width:`${progress/mission.target*100}%`}]} /><View style={styles.trackShine}/></View>
      {ready&&<Pressable onPress={()=>onClaim(mission.id)} style={({pressed})=>[styles.claim,tiny&&styles.claimTiny,pressed&&styles.claimPressed]}><PixelText style={styles.claimText}>CLAIM MANDATE</PixelText></Pressable>}
    </View>
  </View>;
}
...
const styles=StyleSheet.create({
  ...
  headerBand:{backgroundColor:'#0A2731',borderBottomWidth:2,borderBottomColor:colors.brass,paddingHorizontal:10,paddingVertical:8,flexDirection:'row',justifyContent:'space-between',alignItems:'center',gap:8},headerBandMobile:{paddingHorizontal:9,paddingVertical:7},
  card:{backgroundColor:'#123844',padding:11,borderWidth:1,borderColor:'#315E67'},kicker:{color:colors.aqua,fontSize:8,letterSpacing:.9},title:{fontFamily:'PressStart2P',fontSize:8,lineHeight:13,color:colors.cream},titleMobile:{fontSize:7,lineHeight:12},rewards:{alignItems:'flex-end'},rewardsMobile:{minWidth:70},reward:{color:colors.goldLight,fontSize:11},rewardXp:{color:colors.aqua,fontSize:8,marginTop:2},detail:{color:colors.muted,fontSize:9,marginTop:7,lineHeight:14},progressRow:{alignItems:'flex-end',marginTop:6},progressText:{fontSize:8,color:colors.cream},
  ...
  claim:{marginTop:10,backgroundColor:colors.gold,borderWidth:2,borderColor:colors.goldLight,borderBottomWidth:5,borderBottomColor:colors.brassDark,paddingVertical:11,minHeight:48,alignItems:'center',justifyContent:'center'},claimTiny:{minHeight:46},claimPressed:{transform:[{translateY:3}],borderBottomWidth:2},claimText:{fontFamily:'PressStart2P',color:colors.ink,fontSize:6,letterSpacing:.2},
