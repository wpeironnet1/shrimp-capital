...
export function PixelShrimp({ color, accentColor = '#F5B6A9', pattern = 'solid', trait, size = 48, flip = false }: { color: string; accentColor?: string; pattern?: Pattern; trait?: Trait; size?: number; flip?: boolean }) {
  const scale = size / 48;
  return (
...
const styles = StyleSheet.create({
  ...
  leg: { position: 'absolute', backgroundColor: '#F5B6A9' }, antenna: { position: 'absolute', height: 2, backgroundColor: '#F5B6A9' }, crest: { position: 'absolute' }, arm: { position: 'absolute' }, claw: { position: 'absolute', borderWidth: 3, borderLeftWidth: 0, backgroundColor: 'transparent' },
});
