...
export function StockBrowser({state,selectedId,onSelect,onInspect}:{state:GameState;selectedId:string;onSelect:(id:string)=>void;onInspect:(id:string)=>void}){
  ...
  const desktop=width>=960;const mobile=width<700;
  const [filter,setFilter]=useState<Filter>('UNLOCKED');const [sort,setSort]=useState<Sort>('LEVEL');
  ...
  return <View style={styles.shell}>
    ...
    <View style={styles.controlArea}>
      ...
      {hiddenCount>0&&<View style={styles.classified}><PixelText style={styles.classifiedText}>{hiddenCount} CLASSIFIED SPECIES</PixelText></View>}
    </View>
    <View style={styles.contentArea}>{desktop?<View style={styles.grid}>{items.map(card)}</View>:<View><ScrollView horizontal nestedScrollEnabled showsHorizontalScrollIndicator={false} contentContainerStyle={styles.row}>{items.map(card)}<View style={styles.endSpacer}/></ScrollView>{items.length>1&&<View pointerEvents="none" style={styles.scrollCue}><PixelText style={styles.scrollCueText}>SWIPE ►</PixelText></View>}</View>}</View>
  </View>;
}
...
const styles=StyleSheet.create({
  ...
  headerBand:{backgroundColor:'#0B2A34',borderWidth:1,borderColor:'#315965',borderBottomWidth:2,borderBottomColor:colors.brass,padding:12,flexDirection:'row',alignItems:'flex-end',justifyContent:'space-between',gap:12},headerBandMobile:{alignItems:'center'},
  eyebrow:{fontSize:7,color:colors.aqua,letterSpacing:1.5},title:{fontFamily:'PressStart2P',fontSize:10,letterSpacing:.35,marginTop:5,color:colors.cream},titleMobile:{fontSize:8},sub:{fontSize:7,color:colors.muted,marginTop:5},subMobile:{fontSize:7},
  discovery:{alignItems:'flex-end'},discoveryMobile:{minWidth:64},discoveryValue:{fontFamily:'PressStart2P',fontSize:10,color:colors.goldLight},discoveryLabel:{fontSize:6,color:colors.muted,marginTop:3},
  controlArea:{backgroundColor:'#081F28',padding:9,gap:8},controlGroup:{gap:5},controlLabel:{fontSize:6,color:colors.muted,letterSpacing:1.2},controlWrap:{flexDirection:'row',flexWrap:'wrap',gap:6},
  filter:{minHeight:36,paddingVertical:8,paddingHorizontal:11,backgroundColor:'#0D313B',borderWidth:2,borderColor:'#2B5460',justifyContent:'center'},filterActive:{backgroundColor:'#1A5560',borderColor:colors.gold},filterText:{fontSize:7,color:colors.muted},filterTextActive:{color:colors.goldLight},
  sort:{minHeight:34,paddingVertical:7,paddingHorizontal:10,backgroundColor:'#102B34',borderWidth:1,borderColor:'#29454C',justifyContent:'center'},sortActive:{backgroundColor:'#3A3120',borderWidth:2,borderColor:colors.gold},sortText:{fontSize:7,color:'#D9CFB5'},
  classified:{alignSelf:'flex-start',paddingVertical:6,paddingHorizontal:9,backgroundColor:'#171C25',borderWidth:1,borderColor:'#414553'},classifiedText:{fontSize:6,color:'#8E96A2'},
  contentArea:{backgroundColor:'#0A2630',padding:9,borderTopWidth:2,borderTopColor:'#173E48'},grid:{flexDirection:'row',flexWrap:'wrap',gap:9},row:{gap:10,paddingRight:30},
  card:{width:154,minHeight:184,backgroundColor:'#0D3541',borderWidth:2,borderColor:'#315D68',borderBottomWidth:5,borderBottomColor:'#041319',padding:3},cardDesktop:{width:'23.8%',minWidth:170,maxWidth:245},cardMobile:{width:146,minHeight:190},selected:{borderColor:colors.goldLight,backgroundColor:'#123F49'},pressed:{transform:[{translateY:2}],borderBottomWidth:3},
  cardInner:{flex:1,borderWidth:1,borderColor:'#173F49',padding:8,position:'relative'},rarityRail:{position:'absolute',left:0,top:0,bottom:0,width:4},cardTop:{height:68,flexDirection:'row',justifyContent:'space-between',alignItems:'flex-start'},spriteFrame:{width:92,height:62,backgroundColor:'#08242D',borderWidth:2,borderColor:'#2E6671',justifyContent:'center',alignItems:'center',overflow:'hidden'},spriteFrameMobile:{width:86},spriteWater:{position:'absolute',left:0,right:0,bottom:0,height:17,backgroundColor:'#0C425166',borderTopWidth:1,borderTopColor:'#4FABB7'},quantityPill:{backgroundColor:'#061A22',borderWidth:1,borderColor:colors.brass,paddingHorizontal:6,paddingVertical:5},quantityText:{fontSize:7,color:colors.aqua},
  name:{fontFamily:'PressStart2P',fontSize:7,lineHeight:11,minHeight:25,marginTop:8,color:colors.cream},nameMobile:{fontSize:7},metaRow:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:5},rarity:{fontSize:7},level:{fontSize:7,color:colors.muted},valueRow:{flexDirection:'row',alignItems:'baseline',gap:4,marginTop:7},value:{fontFamily:'PressStart2P',fontSize:8,color:colors.goldLight},per:{fontSize:6,color:colors.muted},lineage:{fontSize:6,color:'#8FE0A6',marginTop:5},accessory:{fontSize:6,color:'#C4E9CA',marginTop:4},
  endSpacer:{width:14},scrollCue:{position:'absolute',right:8,bottom:8,backgroundColor:'#061B23E8',borderWidth:1,borderColor:colors.brass,paddingHorizontal:7,paddingVertical:5},scrollCueText:{fontSize:6,color:colors.goldLight},
});
