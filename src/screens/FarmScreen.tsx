...
export function FarmScreen() {
  ...
  const handleFeed = () => {
    ...
    feed(); setFeedingFrenzy(true); setTimeout(() => setFeedingFrenzy(false), 1800);
  };
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.tankWrap}>
        ...
        <View pointerEvents="none" style={styles.overlayClip}>
          ...
          {tintOpacity > 0 && <View style={[StyleSheet.absoluteFillObject,styles.conditionTint,{backgroundColor:tintColor,opacity:tintOpacity}]} />}
          {state.conditions.waterQuality < 62 && <View style={StyleSheet.absoluteFillObject}><View style={[styles.grime,styles.grimeOne]} /><View style={[styles.grime,styles.grimeTwo]} /><View style={[styles.grime,styles.grimeThree]} /></View>}
        </View>
        ...
        {state.ipoCount > 0 && <View pointerEvents="none" style={styles.ipoFixture}>
          <View style={styles.ipoBell}><View style={styles.ipoBellDome}/><View style={styles.ipoBellBase}/></View>
          <View><PixelText style={styles.ipoFixtureTitle}>PUBLIC CO. ×{state.ipoCount}</PixelText><PixelText style={styles.ipoFixtureSub}>{state.prestigeShares} FOUNDER SHARES</PixelText></View>
        </View>}
        {state.ipoCount > 1 && <View pointerEvents="none" style={styles.ipoGoldRail}/>} 
        {totalLineage > 0 && <View pointerEvents="none" style={styles.lineageFixture}>
          <View style={styles.dna}><View style={styles.dnaBarA}/><View style={styles.dnaBarB}/><View style={styles.dnaBarC}/></View>
          <View><PixelText style={styles.lineageFixtureTitle}>GENETICS DESK</PixelText><PixelText style={styles.lineageFixtureSub}>{selectedLineage} pts · {lineageLabel(selectedLineage)}</PixelText></View>
        </View>}

        {feedingFrenzy&&<View pointerEvents="none" style={styles.overlayClip}><PixelText style={styles.frenzyText}>FEEDING FRENZY!</PixelText><View style={[styles.foodPellet,{left:'22%',top:'17%'}]}/><View style={[styles.foodPellet,{left:'37%',top:'11%'}]}/><View style={[styles.foodPellet,{left:'51%',top:'20%'}]}/><View style={[styles.foodPellet,{left:'66%',top:'14%'}]}/><View style={[styles.foodPellet,{left:'78%',top:'23%'}]}/></View>}
        ...
        {ambientEvent==='visitor'&&<View pointerEvents="none" style={styles.overlayClip}><View style={styles.visitorFish}><View style={styles.visitorBody}/><View style={styles.visitorTail}/><View style={styles.visitorEye}/></View><PixelText style={styles.visitorLabel}>VISITING WHALE ANALYST</PixelText></View>}
      ...
      <View style={styles.speciesHeading}><View><PixelText style={styles.sectionTitle}>CURRENT STOCK</PixelText><PixelText style={styles.holdHint}>SELLING, BREEDING & MISSIONS ALL EARN XP</PixelText></View><PixelText style={styles.rarity}>{current.rarity.toUpperCase()}</PixelText></View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.speciesRow}>{species.map((item)=>{const locked=item.unlockLevel>state.level;const acc=state.shrimpAccessories[item.id]??{chain:0,crown:0,visor:0,suit:0};const accessory=acc.suit>0?'suit':acc.visor>0?'visor':acc.crown>0?'crown':acc.chain>0?'chain':undefined;const baseSize=34+(item.rarity==='Exotic'?7:item.rarity==='Mythic'?5:item.rarity==='Legendary'?3:0);const cardSize=Math.min(58,Math.round(baseSize*(item.displayScale??1)));return <Pressable accessibilityRole="button" accessibilityState={{disabled:locked,selected:state.selectedSpecies===item.id}} key={item.id} disabled={locked} onLongPress={()=>setInspectId(item.id)} delayLongPress={350} onPress={()=>selectSpecies(item.id)} style={({pressed})=>[styles.speciesCard,locked&&styles.speciesLocked,state.selectedSpecies===item.id&&styles.speciesSelected,pressed&&!locked&&styles.cardPressed]}><PixelShrimp color={item.color} accentColor={item.accentColor} pattern={item.pattern} trait={item.trait} accessory={accessory} size={cardSize}/><PixelText style={styles.speciesName}>{item.name}</PixelText><PixelText style={styles.speciesRarity}>{item.rarity.toUpperCase()}</PixelText><PixelText style={styles.speciesValue}>{locked?`UNLOCK LV. ${item.unlockLevel}`:`$${Math.round(item.basePrice*valueMultiplier(state,item.id)).toLocaleString()} ea.`}</PixelText>{(state.lineage[item.id]??0)>0&&<PixelText style={styles.lineageTag}>LINEAGE {state.lineage[item.id]} · +{Math.round((lineageMultiplier(state,item.id)-1)*100)}%</PixelText>}{accessory&&<PixelText style={styles.accessoryTag}>{accessory==='visor'?'GREEN VISOR':accessory==='suit'?'SUIT & TIE':accessory.toUpperCase()}</PixelText>}</Pressable>;})}</ScrollView>
